...
function parseCredential(credential: string): { accessKeyId: string; date: string; region: string; service: string } {
...
function conditionValue(fields: Map<string, string>, name: string): string | undefined {
...
function validateConditions(policy: Record<string, unknown>, fields: Map<string, string>, { contentLength }: { contentLength?: number }): { range: { min: number; max: number } | null } {
  ...
  for (const condition of policy.conditions) {
    ...
    for (const [rawName, expected] of Object.entries(condition as Record<string, string>)) {
...
export function verifyPostPolicy({ fields, bucket, lookupCredential, now = Date.now(), contentLength }: {
  ...
  try {
    policy = JSON.parse(Buffer.from(encodedPolicy, 'base64').toString('utf8')) as Record<string, unknown>
  } catch {
...
export function resolveKey(rawKey: string | undefined, filename: string | undefined): string {
